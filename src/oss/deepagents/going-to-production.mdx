...
config = {"configurable": {"thread_id": str(uuid7())}}  # [!code highlight]
agent.invoke(
    {"messages": [{"role": "user", "content": "Plan a 3-day trip to Tokyo"}]},
    config=config,  # [!code highlight]
    context=Context(user_id="user-123"),  # [!code highlight]
)
...
// Start a conversation
const config = { configurable: { thread_id: uuidv4() } };  // [!code highlight]
await agent.invoke(
  { messages: [{ role: "user", content: "Plan a 3-day trip to Tokyo" }] },
  { ...config, context: { userId: "user-123" } },  // [!code highlight]
);
