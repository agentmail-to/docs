...
You can then click into a particular thread. You can view the thread in two different ways:

- **Messages** view: the conversation layer. Scan each turn as a chat-style thread showing user and assistant messages, tool calls, and subagent activity.
...
Switch between views using the buttons at the top of the page or keyboard shortcuts `M` (Messages) and `D` (Details). The right panel shows stats for the thread, including turn count, first and last start times, P50/P99 latency, and a cost breakdown by input and output tokens. For a full description of both views, see [View traces](/langsmith/view-traces).
...
On the **Threads** tab of a project, you can save commonly used filters the same way you would at the project level. [Set a filter](/langsmith/filter-traces-in-application#create-and-apply-filters) using the **Add filter** button, then click **Save view**.
