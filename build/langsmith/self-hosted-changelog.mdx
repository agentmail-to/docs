...
- Fixed layout issue in automations table to prevent header truncation.
- Added solid icons and xs variant to the icon package in the UI.
- Improved sandbox build process with Docker build fix.
- Updated details view header for a better user interface.
- Removed unnecessary join from session stats queries to improve performance.
- Made scrollable filter dropdown in MCP server frontend.
- Added user and agent cost breakdown tables.
- Implemented responsive and performance improvements in UI and frontend layouts.
- Introduced URL allowlist enforcement for improved JWT injection security.
- Enabled feedback key and resource filters in evaluators table for better sorting.
- Fixed bug causing icon sticking during interaction in UI.
- Improved evaluator tables with searchable dropdowns, dynamic sorting, and new metadata.
- Added support for asynchronous agents in fleet system.
- Enhanced frontend with a dialog system and analytics for tracking template selections.
- Fixed loading and responsiveness issues in several frontend components.
- Extended tracing project features and functionality in run rules.
- Improved evaluator reuse functionalities and sorting options.
- Added support for new model integrations and tools in agent-builder.
- Provided dark mode gradient fix and enhanced frontend style elements.
- Enabled mobile-friendly login and installation of PWA for better device compatibility.
- Enhanced frontend usability with detail page improvements and filter state persistence.
- Introduced new authentication and security features including single sign-on (SSO) login button.
- Provided click-to-filter capabilities for evaluator fields in the frontend.
- Implemented JWT-based local self-hosted license updates.
- Added metrics for dashboarding and usage insights across agent and fleet projects.
- Improved playground performance by reducing unnecessary re-renders.
...
- Fixed UI truncation issue by widening the 'Enabled' column in the automations table.
- Improved evaluator sorting and filtering by adding created and updated timestamps.
- Fixed logout process to correctly terminate IdP sessions for OIDC.
- Fixed issue where clicking the back arrow would close evaluator pane.
- Added backward pagination from selected trace/run in thread view.
- Improved Run Detail Headers - Reduced padding on detail page headers.
- Enabled audit logging for all remaining Go write endpoints.
- Introduced keyboard shortcut (cmd/ctrl+s) to save on editor page.
- Added support for GLM5 model and other model upgrades and integrations.
- Enhanced security with JWT for various integrations, including Bedrock compatibility. 
- Improved login page to be mobile-friendly and installable as a PWA.
- Added Postgres TLS migrations with support for system root cert.
- Introduced a toggle to auto-approve memory updates in the interrupt UI.
- Migrated CLI for standard operations with adjustable S3 addressing style for bulk exports.
- Enabled sorting for annotation queues list view columns.
- Added support for mTLS in clickhouse migrations.
- Added Spend Limit features with improved spend breakdown UI and visibility.
- Fixed dark mode gradient and header element placement.
- Ensured PostgreSQL IAM role for audit logs.
- Provided 'Run Now' feature to show error toast on failure.
- Incorporated performance optimizations for trace layout rendering and sessions list retrieval.
- Added feature flag (all-scopes auth) option in auth UI.
- Enhanced evaluator details page with Source section and UX improvements.
- Enabled tracing project deduplication by ID and adjusted rendering styles to improve UX.
- Improved the performance for page responsiveness and loading speed for large data entries.
